...
# Calculate a Zero Cross for a given dataframe
def calc_zero_cross(dataframe, column):
    """
    Function to calculate a zero cross event on a dataframe. The column name indicates the column to compare zero
    against
    :param dataframe: dataframe object with data to be analyzed
    :param column: string of the column name
    :return: dataframe with cross events
    """
    # Create a position column
    dataframe['position'] = dataframe[column] > 0
    # Create a pre_position column which is the previous row shifted by 1
    dataframe['pre_position'] = dataframe['position'].shift(1)
    # Drop any NA values
    dataframe.dropna(inplace=True)
    # Define a zero cross event
